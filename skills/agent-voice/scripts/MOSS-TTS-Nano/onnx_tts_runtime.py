...
class OnnxTtsRuntime(OrtCpuRuntime):
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        # torchaudio 2.9+ 移除 soundfile 后端并强制走 torchcodec；torchcodec 依赖系统
        # FFmpeg 动态库，FFmpeg 8/9 下对部分 WAV 解码失败（Invalid data）。
        # WAV/FLAC 用 soundfile 读取（channels-first 语义与 torchaudio.load 一致），其余格式回落 torchcodec。

...
class CoStormAutonomousResearchWorkflowGeneratedTests(unittest.TestCase):
 ...
 'evidence_registry': ['[1] source-a',
    ...
    def test_expert_results_accepts_multiple_unused_evidence(self):
        result = verifiers.verify_launch_expert_subagents(
            ...
            step_id='launch_expert_subagents',
            observation={'status': 'succeeded',
 'summary': 'One expert returned multiple well-formed evidence items that were not cited in its '
            'summary or artifact.',
 'structured_output': {'expert_round_index': 1,
                       'expert_results': [{'expert_id': 'historian',
                                           'summary': 'Historian perspective completed with '
 ...
 'structured_output': {'expert_roster': [{'id': 'historian',
    ...
    def test_roundtable_diagnostics_hint_on_truncated_expert_results(self):
        result = verifiers.verify_autonomous_roundtable(
            repo_root=str(REPO_ROOT),
            run_id="generated-test-run",
            step_id='autonomous_roundtable',
            observation={'status': 'succeeded',
 'summary': 'Round with truncated persisted expert package.',
 'structured_output': {'last_turn_summary': 'turn',
                       'conversation_transcript': ['turn1', 'turn2'],
                       'evidence_registry': ['[1] loc — claim one',
                                             '[2] loc — claim two',
                                             '[3] loc — claim three'],
                       'coverage_map': ['软件栈与控制架构', '致动器选型'],
                       'coverage_assessment': [{'topic_id': '软件栈与控制架构',
                                                'status': 'covered',
                                                'evidence_refs': ['[1]'],
                                                'open_gaps': [],
                                                'next_validation_metrics': []},
                                               {'topic_id': '致动器选型',
                                                ...
                                                'open_gaps': [],
                                                'next_validation_metrics': []}],
                       'coverage_decision_rationale': 'coverage ok',
                       'next_round_validation_plan': [],
                       'report_scope_status': 'complete',
                       'knowledge_map_summary': 'map',
                       'expert_roster': [{'id': 'e1', 'role': 'r', 'brief': 'b'}],
                       'round_index': 2,
                       'round_decision': 'report',
                       'continue_roundtable': False,
                       'should_reorganize': False,
                       ...
                       'ready_for_report': True}},
            state={'round_index': 1,
 'constraints': {'max_rounds': 4, 'coverage_threshold': 2, 'min_evidence_items': 3},
 'expert_results_complete': True,
 'expert_roster': [{'id': 'e1', 'role': 'r', 'brief': 'b'}],
 'conversation_transcript': ['turn1'],
 'evidence_registry': ['[1] loc — claim one', '[2] loc — claim two', '[3] loc — claim three'],
 'coverage_map': ['软件栈与控制架构', '致动器选型'],
 'coverage_assessment': [{'topic_id': '软件栈与控制架构',
                          'status': 'covered',
                          'evidence_refs': ['[1]'],
                         ...
                         {'topic_id': '致动器选型',
                          ...
                          'open_gaps': [],
                          'next_validation_metrics': []}],
 'serialization_diagnostics': {'dropped_fields': ['expert_results'],
                               'dropped_at': 'graph_state_serialize'}},
        )
        self.assertIs(result['passed'], False)

    def test_roundtable_accepts_completed_expert_package(self):
        result = verifiers.verify_autonomous_roundtable(
            repo_root=str(REPO_ROOT),
            run_id="generated-test-run",
            step_id='autonomous_roundtable',
            observation={'status': 'succeeded',
 'summary': 'Round with a complete persisted expert package.',
 'structured_output': {'last_turn_summary': 'turn',
                       'conversation_transcript': ['turn1', 'turn2'],
                       'evidence_registry': ['[1] loc — claim one',
                                             '[2] loc — claim two',
 ...
 'constraints': {'max_rounds': 4, 'coverage_threshold': 2, 'min_evidence_items': 3},
 'expert_results_complete': True,
 'expert_results': [{'expert_id': 'e1', 'summary': 'ok', 'artifact_path': 'a', 'new_evidence': []}],
 'expert_roster': [{'id': 'e1', 'role': 'r', 'brief': 'b'}],
 'conversation_transcript': ['turn1'],
 'evidence_registry': ['[1] loc — claim one', '[2] loc — claim two', '[3] loc — claim three'],
 'coverage_map': ['软件栈与控制架构', '致动器选型'],

...
- When context.output_dir is set, write the report artifact under that directory. output_dir and report_path may be repository-relative POSIX paths or absolute POSIX paths; report_path must resolve inside output_dir. Reject backslashes, control characters, '.' or '..' path segments, and symbolic links.

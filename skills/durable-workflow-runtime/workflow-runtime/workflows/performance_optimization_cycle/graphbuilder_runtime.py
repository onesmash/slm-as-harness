...
NODE_DEFINITIONS = {
    ...
    "research_optimization": NodeDefinition(
        ...
        expected_artifact="research brief, implementation-ready change summary, verification plan, and evidence map",
        resume_instructions="Return an Observation preserving run_id and step_id.",
    ),
...
def _template_context_from_state(state: workflow_state.PerformanceOptimizationCycleWorkflowState) -> dict:
    ...
    context.update(
        ...
        "submission_test_command": _format_prompt_value(state.submission_test_command),
        "submission_tests_passed": _format_prompt_value(state.submission_tests_passed),
        "ready_for_review": _format_prompt_value(state.ready_for_review),

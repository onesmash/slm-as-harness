...
NODE_DEFINITIONS = {
    ...
    "request_final_code_review": NodeDefinition(
        step_id="request_final_code_review",
        prompt_asset_path=PROMPTS_DIR / "request_final_code_review.md",
        intent="review_merged_final_state",
        expected_artifact="merged-final review findings and merge-safety decision",
        resume_instructions="Return an Observation preserving run_id and step_id.",
    ),
    "write_code_kb_feedback": NodeDefinition(
        step_id="write_code_kb_feedback",
        prompt_asset_path=PROMPTS_DIR / "write_code_kb_feedback.md",
        intent="write_knowledge_base_feedback",
        expected_artifact="knowledge-base page/backlog/QA feedback updates for the completed change",
        resume_instructions="Return an Observation preserving run_id and step_id.",
    ),
...
def _template_context_from_state(state: workflow_state.IosAiAssistedDevelopmentFlowWorkflowState) -> dict:
    ...
    context.update(
        ...
        "kb_updated": _format_prompt_value(state.kb_updated),
        "updated_pages": _format_prompt_value(state.updated_pages),
        "qa_feedback_path": _format_prompt_value(state.qa_feedback_path),
        "kb_checks": _format_prompt_value(state.kb_checks),
        ...
        "mr_url": _format_prompt_value(task_input_values.get("mr_url")),
        "kb_scope": _format_prompt_value(task_input_values.get("kb_scope")),

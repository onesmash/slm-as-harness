...
NODE_DEFINITIONS = {
    "run_brainstorming": NodeDefinition(
        ...
        expected_artifact="an approved, review-complete design package with a written design document path",
    ...
    "write_implementation_plan": NodeDefinition(
        ...
        expected_artifact="approved implementation plan document and selected execution mode",
        resume_instructions="Return an Observation preserving run_id and step_id.",
    ),
    "approve_plan": NodeDefinition(
        step_id="approve_plan",
        prompt_asset_path=PROMPTS_DIR / "approve_plan.md",
        intent="confirm_plan_and_execution_mode",
        expected_artifact="explicit user approval to execute the plan with the selected superpowers mode",
        resume_instructions="Return an Observation preserving run_id and step_id.",
    ),
...
def _template_context_from_state(state: workflow_state.IosAiAssistedDevelopmentFlowWorkflowState) -> dict:
    ...
    context.update(
        ...
        "plan_user_approved": _format_prompt_value(state.plan_user_approved),
        "plan_user_feedback": _format_prompt_value(state.plan_user_feedback),

...
Regeneration keeps files that are outside the creator-owned generated paths,
such as workflow-specific support modules and extra fixtures. Symlinked custom
files are skipped with a warning. Files owned by the spec generator, including
`state.py` and the generated `tests/test_workflow.py`, are regenerated by
default; set the workflow-local spec field `state_mode` to `custom` when the
workflow has a domain-owned state implementation, and the existing `state.py`
is carried forward during `--force` regeneration. Custom state does not receive
the generated state's generic repair regression cases; declare domain-specific
state, recovery, and prompt-context coverage in `regression_tests`. Rerun the
workflow-specific tests and review custom state validation and size bounds after
regeneration.
...
- Mark stages that cannot proceed without an authoritative verifier with
  `require_passing_verifier: true`; add `missing_verifier_route` when a
  recovery stage must preserve a workflow-specific fail-closed destination.
...
- Use `verifier_templates` for common complex checks such as list item required
  keys, conditional required fields, uniqueness, fixed or constraint-driven
  minimum counts, safe artifact-list policies, unresolved-finding gates, and
  artifact section checks when the DSL expresses the invariant cleanly. When the
